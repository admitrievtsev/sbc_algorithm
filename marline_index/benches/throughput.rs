...
use marline_index::sketch::U64Sketch;
use marline_sketcher::odess_hasher::{IndexType, OdessHasher, SuperFeatureConfig};
...
fn decompress_bz2(path: &str) -> Vec<u8> {
    let compressed = std::fs::read(path).unwrap_or_else(|e| {
        panic!("Failed to read {}: {}", path, e);
    });
    let mut decoder = bzip2::read::BzDecoder::new(compressed.as_slice());
    let mut data = Vec::new();
    decoder.read_to_end(&mut data).unwrap_or_else(|e| {
        panic!("Failed to decompress {}: {}", path, e);
    });
    data
}

fn extract_kernel_versions(data: &[u8]) -> Vec<Vec<u8>> {
    let mut archive = tar::Archive::new(data);
    let mut versions: std::collections::HashMap<String, Vec<u8>> = std::collections::HashMap::new();
    ...
    for entry in archive.entries().expect("Failed to read tar entries") {
        ...
        let version = if path.starts_with("linux-3.4.5/") {
            "linux-3.4.5"
        } else if path.starts_with("linux-3.4.6/") {
            "linux-3.4.6"
        } else if path.starts_with("linux-3.4.7/") {
            "linux-3.4.7"
        } else {
            continue;
        };
        ...
        if entry.header().entry_type() == tar::EntryType::Regular {
            ...
            if entry.read_to_end(&mut content).is_ok() {
                versions.entry(version.to_string()).or_default().extend(content);
            }
        }
    }

    let mut result: Vec<Vec<u8>> = Vec::new();
    for v in &["linux-3.4.5", "linux-3.4.6", "linux-3.4.7"] {
        if let Some(data) = versions.get(*v) {
...
fn hash_to_sketch(hasher: &OdessHasher, chunk: &[u8]) -> U64Sketch<6> {
    let hash = hasher.calculate_hash(chunk);
    let features = hash.as_slice();
    let mut arr = [0u64; 6];
...
fn bench_throughput(c: &mut Criterion) {
    let bz2_path = format!("{}/../kernels.tar.bz2", env!("CARGO_MANIFEST_DIR"));
    let data = decompress_bz2(&bz2_path);
    eprintln!("Decompressed: {} bytes ({:.1} MB)", data.len(), data.len() as f64 / 1e6);

    let versions = extract_kernel_versions(&data);
    eprintln!("Extracted {} kernel versions", versions.len());
    for (i, v) in versions.iter().enumerate() {
        eprintln!("  Version {}: {} bytes ({:.1} MB)", i + 1, v.len(), v.len() as f64 / 1e6);
    }

    let hasher = OdessHasher::new(7, IndexType::SuperFeatured(SuperFeatureConfig::new(2, 6)));

    let chunks_v1: Vec<(&[u8], U64Sketch<6>)> =
        gear_chunking(&versions[0]).into_iter().map(|c| (c, hash_to_sketch(&hasher, c))).collect();
    eprintln!("V1: {} chunks", chunks_v1.len());

    let chunks_v2: Vec<(&[u8], U64Sketch<6>)> =
        gear_chunking(&versions[1]).into_iter().map(|c| (c, hash_to_sketch(&hasher, c))).collect();
    eprintln!("V2: {} chunks", chunks_v2.len());

    let chunks_v3: Vec<(&[u8], U64Sketch<6>)> =
        gear_chunking(&versions[2]).into_iter().map(|c| (c, hash_to_sketch(&hasher, c))).collect();
    eprintln!("V3: {} chunks", chunks_v3.len());

    // Phase 1: put v1 into index with unique keys
    let index = BenchIndex::new(IndexStorage::new());
    for (i, (_, sketch)) in chunks_v1.iter().enumerate() {
        index.put(&(i as u64), *sketch).unwrap();
    }

    // Phase 2: put v2 into index with following keys
    let offset = chunks_v1.len();
    for (i, (_, sketch)) in chunks_v2.iter().enumerate() {
        index.put(&((offset + i) as u64), *sketch).unwrap();
    }

    let total_query_bytes: u64 = chunks_v3.iter().map(|(c, _)| c.len() as u64).sum();
    eprintln!("V3 query data: {:.1} MB", total_query_bytes as f64 / 1e6);
    ...
    group.bench_function("kernels_6sf", |b| {
        b.iter(|| {
            for (_, sketch) in &chunks_v3 {
                let r = std::hint::black_box(index.top_k(sketch, 10).unwrap());
    ...
    let linear_index = LinearSearchIndex::new();
    for (i, (_, sketch)) in chunks_v1.iter().enumerate() {
        linear_index.put(&(i as u64), *sketch).unwrap();
    }
    let offset = chunks_v1.len();
    for (i, (_, sketch)) in chunks_v2.iter().enumerate() {
        linear_index.put(&((offset + i) as u64), *sketch).unwrap();
    }

    let mut group = c.benchmark_group("linear_throughput");
    group.sample_size(10);
    group.throughput(Throughput::Bytes(total_query_bytes));
    group.bench_function("kernels_6sf", |b| {
        b.iter(|| {
